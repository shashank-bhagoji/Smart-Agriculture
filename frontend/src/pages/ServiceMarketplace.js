          ...
          <div className="card" style={{ maxWidth: "420px", width: "90%", padding: "1.5rem", borderRadius: "20px", boxShadow: "0 25px 50px -12px rgba(0,0,0,0.5)", position: "relative" }}>
            ...
            <form onSubmit={confirmHiring} style={{ display: "flex", flexDirection: "column", gap: "0.8rem", marginTop: "1rem" }}>
              <div className="input-group">
                <label style={{ display: "block", marginBottom: "0.3rem", fontSize: "0.9rem", color: "var(--text-muted)" }}>{t('service_date')}</label>
                <input 
                  type="date" 
                  required 
              ...
              <div style={{ padding: "0.8rem", background: "rgba(16, 185, 129, 0.08)", borderRadius: "10px", border: "1px solid rgba(16, 185, 129, 0.1)", fontSize: "0.9rem" }}>
                ...
                <p style={{ margin: "0.5rem 0 0 0", color: "var(--primary)", fontWeight: "700", fontSize: "1.05rem" }}>{t('total_cost')}: ₹{selectedItem?.price * bookingDetails.days}</p>
              </div>
